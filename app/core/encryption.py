...
def decrypt_refresh_token(encrypted: str) -> str:
    """Decrypt a refresh token that was encrypted by FojiApi's EncryptionService."""
    parts = encrypted.split(":")
    if len(parts) != 3:
        raise ValueError("Invalid encrypted token format. Expected: base64(iv):base64(ciphertext):base64(tag)")
